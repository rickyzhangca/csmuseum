...
import { singularAssetType, urlTypeNames } from '@/utils';
...
export const ContentDetails = ({
  ...
  const getContentDetail = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from(`${contentType}_with_creators`)
        .select('*')
        .eq('id', contentId)
        .single();
  ...
  return (
    <div className="flex flex-col">
      ...
      {content.shots_count && (
        <div className="max-w-8xl relative z-10 mx-auto w-full rounded-t-4xl bg-black/95 px-16 py-16 shadow-[0_-24px_24px_-8px_rgba(0,0,0,0.08)]">
          <div className="3xl:grid-cols-3 grid grid-cols-1 gap-6 xl:grid-cols-2">
            {Array.from({ length: content.shots_count }).map((_, index) => (
              <button
                type="button"
                key={`${contentId}-shot-${
              ...
              >
                <img
                  src={`${import.meta.env.VITE_BUNNY_CDN_URL}/${contentType}/${contentId}/${index}.webp`}
                  loading="lazy"
